                                                ...
                                                return (
                                                    <div key={index} className="flex justify-between mt-2">
                                                        <div className="flex items-center gap-2">
                                                            <div
                                                                className="text-sm text-gray-500">{shortenAddress(item.contract_address)}</div>
                                                            <ExternalLink className={'text-gray-500'} size={16}/>
                                                        </div>
                                                        <div
                                                            className="text-sm text-gray-500">{item.selector_name}</div>
                                                    </div>
                                                )
                                            })}
                                        </div>
                                        <div
                                            className="text-sm text-gray-500 mt-4">{formatTime(it.node.timestamp)}</div>
                                    </CardContent>
                                </Card>
                            )
                        })
