...
export default function OwnappSetting() {
    ...
    const [dappList, setDappList] = useState(DappList?.map(it => {
        return {
            ...it,
            selected: false
        }
                ...
                <div className={'grid grid-cols-3 gap-4 mt-8'}>
                    {
                        dappList?.map(it => {
                            return (
                                <div key={it.name} className={'flex flex-col gap-4 cursor-pointer relative'} onClick={() => {
                                    setDappList(dappList.map(item => {
