...
export async function queryNFTBalance(contractAddress: string) {
  const body = {
    query:
      'query NftsTableQuery(\n  $first: Int!\n  $after: String\n  $input: NftsInput!\n) {\n  ...NftsTablePaginationFragment_nfts_2DAjA4\n}\n\nfragment NftsTablePaginationFragment_nfts_2DAjA4 on Query {\n  nfts(first: $first, after: $after, input: $input) {\n    edges {\n      node {\n        id\n        nft_contract_address\n        token_id\n        name\n        image_url\n        __typename\n      }\n      cursor\n    }\n    pageInfo {\n      endCursor\n      hasNextPage\n    }\n  }\n}\n',
    variables: {
      first: 30,
      after: null,
      input: {
        sort_by: 'minted_at_timestamp',
        order_by: 'desc',
...
export const fetchTokenInfo = async (address: string, user?: string) => {
  try {
    const body = {
      query:
        'query TokenPageQuery(\n  $input: TokenContractInput!\n) {\n  tokenContract(input: $input) {\n    token_contract_address\n    ...TokenPageOverviewTabFragment_tokenContract\n    contract {\n      ...ContractFunctionReadWriteTabFragment_contract\n      id\n    }\n    id\n  }\n}\n\nfragment ContractFunctionReadCallsFragment_starknetClass on StarknetClass {\n  is_code_verified\n  abi_final\n}\n\nfragment ContractFunctionReadWriteTabFragment_contract on Contract {\n  contract_address\n  starknet_class {\n    ...ContractFunctionReadCallsFragment_starknetClass\n    ...ContractFunctionWriteCallsFragment_starknetClass\n    id\n  }\n}\n\nfragment ContractFunctionWriteCallsFragment_starknetClass on StarknetClass {\n  is_code_verified\n  abi_final\n}\n\nfragment TokenPageOverviewTabFragment_tokenContract on TokenContract {\n  token_contract_address\n  name_custom\n  name\n  symbol\n  decimals\n  is_social_verified\n  token_contract_stats {\n    total_supply_display\n    number_of_owners\n    id\n  }\n}\n',
      variables: {
        input: {
    ...
    if (data.data) {
      const contract = data.data?.tokenContract;
      let balance = '';
      if (user) {
        const contractIns = new Contract(
          contract.contract.starknet_class.abi_final,
          contract.token_contract_address,
          provider
        );

        const balanceWei = await contractIns.balance(user);

        balance = formatUnits(balanceWei, contract.decimals);
      }
      return {
        name: contract?.name,
        contractAddress: contract?.token_contract_address,
        symbol: contract?.symbol,
        decimals: contract?.decimals,
        totalSupply: contract?.token_contract_stats?.total_supply_display,
        balance,
        icon: TokenUrlMap.ERC20
      };
    } else {
